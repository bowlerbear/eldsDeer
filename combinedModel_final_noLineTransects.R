...
library(mgcv)
jags.ready <- jagam(Deer.ct~s(x, y,k=5), 
...
bugs.data<-list(#camera trap data
                site.ct = sites.ct,
                y.ct = myGridDF3km$Deer.ct[sites.ct],
                ...
                Fields=as.numeric(scale(sqrt(myGridDF3km$Fields+1)))[sites.ct],
                Military=myGridDF3km$MilitaryF[sites.ct],
                Villages=as.numeric(scale(sqrt(myGridDF3km$Villages+1)))[sites.ct],
                Water=waterMatrix,
                Month=monthMatrix,
                #spatial covariates
                X = X,
...
cat("
    ...
    ## Parametric effect priors CHECK tau=1/6^2 is appropriate!
...
params <- c("rho","intercept.ct","mean.p.ct","beta.forest","beta.military","beta.fields","beta.villages",
            "psi","theta.water","alpha.month","fit","fit.new")
